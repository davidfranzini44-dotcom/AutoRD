...
import { listBankTeam, createBankUser, setBankUserPermissions, setBankUserActive } from '../data/api'

// Granular access for a bank's own analysts (mirrors the dealer team model).
const PERMS = [
  { key: 'solicitudes', label: 'Solicitudes', desc: 'Revisar y decidir solicitudes de crédito' },
  { key: 'tasas', label: 'Tasas y reglas', desc: 'Configurar APR por combustible y reglas de plazo' },
  { key: 'whatsapp', label: 'WhatsApp', desc: 'Bandeja de mensajes' },
  { key: 'reportes', label: 'Reportes', desc: 'Métricas y reportes del banco' },
  { key: 'equipo', label: 'Equipo', desc: 'Crear y administrar usuarios del banco' },
]
...
export default function BankTeam() {
  ...
  useEffect(() => {
    let alive = true
    setLoading(true)
    listBankTeam().then((t) => { if (alive) { setTeam(t); setLoading(false) } }).catch(() => { if (alive) setLoading(false) })
    return () => { alive = false }
  }, [reload])
  ...
  async function togglePerm(m, key) {
    ...
    setBusyId(m.id)
    try { await setBankUserPermissions(m.id, permissions); refetch() }
    catch (e) { alert(e?.message || 'No se pudo actualizar') }
    finally { setBusyId(null) }
  }
  async function toggleActive(m) {
    setBusyId(m.id)
    try { await setBankUserActive(m.id, !m.active); refetch() }
    catch (e) { alert(e?.message || 'No se pudo actualizar') }
    finally { setBusyId(null) }
  }

  const owners = team.filter((m) => m.bank_role === 'owner')
  const employees = team.filter((m) => m.bank_role !== 'owner')

  return (
    <div>
      <div className="admin-head">
        <div>
          <h1 style={{ fontSize: 22 }}>Equipo del banco</h1>
          <p className="tiny muted">Crea cuentas para tus analistas y define a qué pueden acceder</p>
        </div>
        <button className="btn btn-primary" onClick={() => { setCreated(null); setAdding(true) }}><UserPlus size={17} /> Agregar usuario</button>
      </div>
        ...
        <>
          {owners.map((m) => (
            <div className="card card-pad" key={m.id} style={{ marginBottom: 12 }}>
              <div className="row center gap-10">
                ...
                <div className="grow">
                  <div className="strong">{m.full_name || 'Administrador'}</div>
                  <div className="tiny muted">{m.email} · Administrador (acceso total)</div>
                </div>
                <span className="chip chip-teal">Administrador</span>
              </div>
            </div>
          ))}
          ...
          <div className="card">
            <div className="row between center" style={{ padding: '14px 16px 6px' }}>
              <h3 style={{ fontSize: 15 }}>Analistas ({employees.length})</h3>
            </div>
            {employees.length === 0 ? (
              <div className="muted tiny" style={{ padding: '4px 16px 18px' }}>Aún no has agregado usuarios. Crea el primero con “Agregar usuario”.</div>
            ) : (
          ...
          <div className="tiny muted row center gap-6" style={{ marginTop: 12 }}>
            <Shield size={13} /> Cada usuario inicia sesión con su correo. Tú controlas su acceso desde aquí.
          </div>
        </>
      )}
...
function AddUserModal({ onClose, onCreated }) {
  ...
  const [permissions, setPermissions] = useState({ solicitudes: true, tasas: false, whatsapp: true, reportes: true, equipo: false })
  ...
  async function submit() {
    ...
    setSaving(true)
    try {
      const res = await createBankUser({ fullName: fullName.trim(), email: email.trim(), permissions })
      onCreated({ email: email.trim(), tempPassword: res.tempPassword })
    } catch (e) {
      setErr(e?.message === 'email_invalido' ? 'Correo inválido' : (e?.message || 'No se pudo crear el usuario'))
      setSaving(false)
    }
  }
    ...
    <ModalShell title="Agregar usuario" onClose={onClose}>
      <div className="col gap-12" style={{ padding: 18 }}>
        ...
        <label className="col gap-4"><span className="tiny strong">Correo electrónico</span>
          <input className="input" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="analista@banco.com" /></label>
        <div>
...
function CredentialsModal({ created, onClose }) {
  ...
  return (
    <ModalShell title="Usuario creado" onClose={onClose}>
      <div className="col gap-12" style={{ padding: 18 }}>
        <div className="notice" style={{ borderColor: 'var(--green-bd)', background: 'var(--green-bg)' }}>
          <KeyRound size={16} /><span>Comparte estas credenciales con el usuario. La contraseña temporal <strong>no se volverá a mostrar</strong>.</span>
        </div>
